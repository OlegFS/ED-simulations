...
def run(params,sim_time=50000):
    ...
    tau_m = np.zeros([N])
    tau_m[:NE] = params['tau_m']
    tau_m[NE:] = 10.0 #Inhibitory neurons
    # Process the parameters
    nu_th = Vthr / (Jext*tau_m[0]*kE)
    nu_ex = eta * nu_th
    p_rate = nu_ex
    ...
    while t<sim_time:
        ...
        if (t-previous_st[n])>tau_ref: # Refactory period
            if event=='I':
                ...
                #update adaptation
                V_= V_update(t,et[n],Vm[n],tau_m[n])
...
if __name__ == "__main__":
    ...
    Gs = [9.0,900/100,700/300,1.0,300/700,200/800,200/800]
    #for i,epsilon in enumerate([0.05,0.1,0.3,0.5,0.7,0.8,0.95]):
    Es = [0.05,0.1,0.3,0.5,0.7,0.8,0.95]
    ...
    print(params)
    sim_time = 50000
    result = run(params,sim_time)#[pool.apply_async(run,args = (par)) for par in all_params]
